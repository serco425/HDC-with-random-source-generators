function message = SIMPLE_UPDATE_langRecognition_LFSR_8192
...
function randomHV = genRandomHV(D, key)
if mod(D,2)
    disp ('Dimension is odd!!');
else
    seed = [false false false false true false true true false false false false false];
    randomHV = LFSR_8192(seed, (D/2), D, key);
end
end
...
if itemMemory.isKey (key)
    ...
    itemMemory(key) = genRandomHV (D, key);
    randomHV = itemMemory (key);
end
end
...
for i = 1:1:length(langLabels)
    fileAddress = strcat(pathstr, '\training_texts_SC\', langLabels (i),'.txt');
